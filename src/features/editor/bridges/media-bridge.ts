  ...
  async importFile(
    ...
    try {
      const result = await this.mediaImportService.importMedia(file, {
        ...
        // 30 thumbs at import time gives enough resolution for zooming up to
        // ~3x without visibly repeating frames. The buildStripFromExistingThumbs
        // path picks the nearest existing thumb for each slot, so higher counts
        // here directly improve zoom-in fidelity without re-extraction cost.
        thumbnailCount: 30,
        thumbnailWidth: 160,
