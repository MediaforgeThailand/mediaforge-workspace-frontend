...
import { AudioEffectsEngine, getAudioEffectsEngine } from "@/lib/openreel-core";
...
export class AudioBridge {
  ...
  private realtimeAudioGraph: RealtimeAudioGraph | null = null;
  private initialized = false;
  private unsubscribeProject: (() => void) | null = null;
  /**
   * Snapshot of the volume value last pushed to the realtime audio graph for
   * each clip. Used to detect deltas in the project subscription so we only
   * call `setClipVolume` when the value actually changes (instead of on every
   * project mutation). Cleared when the bridge is disposed.
   */
  private lastSyncedVolumes: Map<string, number> = new Map();
  ...
  async initialize(): Promise<void> {
    ...
    this.realtimeAudioGraph =
      engineState.playbackController?.getRealtimeAudioGraph() ?? null;

    if (this.realtimeAudioGraph) {
      this.setupClipVolumeSubscription();
      // Seed the cache + graph with current volumes so any clip already in
      // the project at bridge-init time has its override set before the
      // scheduler runs.
      this.syncAllClipVolumes(useProjectStore.getState().project);
    }
  ...
  /**
   * Subscribe to project changes so any volume mutation in the store — from
   * the Inspector slider, the rubberband drag, undo/redo, or programmatic
   * actions like `detachAudio` — is forwarded to the realtime audio graph.
   * This is the core fix for Bug 2 / Bug 3: the graph was previously frozen
   * at the volume baked in at schedule time.
   */
  private setupClipVolumeSubscription(): void {
    this.unsubscribeProject = useProjectStore.subscribe(
      (state) => state.project,
      (project) => {
        if (!this.realtimeAudioGraph) return;
        this.syncAllClipVolumes(project);
      },
    );
  }

  private syncAllClipVolumes(project: Project): void {
    if (!this.realtimeAudioGraph) return;
    ...
    for (const track of project.timeline.tracks) {
      ...
      for (const clip of track.clips) {
        seenIds.add(clip.id);
        const newVolume = clip.volume ?? 1;
        const last = this.lastSyncedVolumes.get(clip.id);
        if (last !== newVolume) {
          this.lastSyncedVolumes.set(clip.id, newVolume);
          this.realtimeAudioGraph.setClipVolume(clip.id, newVolume);
        }
      }
    }
    // Drop tracked volumes for clips that no longer exist (e.g. user deleted
    // a track) so the cache doesn't grow unbounded.
    for (const id of Array.from(this.lastSyncedVolumes.keys())) {
      if (!seenIds.has(id)) this.lastSyncedVolumes.delete(id);
    }
  }

  /**
   * Fast-path setter for live drag interactions. Updates the realtime audio
   * graph immediately without touching the Zustand store. The drag handler
   * should commit the final value to the store on pointerup (which will
   * trigger one project update, and the subscription above will see it
   * matches `lastSyncedVolumes` and no-op).
   *
   * Bypassing the store avoids the cascade of:
   *   pointermove -> store update -> playback-bridge subscription ->
   *   playbackController.setProject -> clearAudioBuffer/stopAudioPlayback ->
   *   React re-render of all consumers of `project`
   * which previously fired at ~60 Hz during the drag.
   */
  setClipVolumeRealtime(clipId: string, volume: number): void {
    if (!this.realtimeAudioGraph) return;
    this.realtimeAudioGraph.setClipVolume(clipId, volume);
    this.lastSyncedVolumes.set(clipId, volume);
  }
  ...
  dispose(): void {
    ...
    this.realtimeAudioGraph = null;
    this.lastSyncedVolumes.clear();
...
export async function initializeAudioBridge(): Promise<AudioBridge> {
  ...
  // Dev-only: expose the bridge on `window` so Playwright self-tests can
  // verify volume changes hit the realtime audio graph end-to-end.
  if (
    typeof window !== "undefined" &&
    typeof import.meta !== "undefined" &&
    (import.meta as { env?: { DEV?: boolean } }).env?.DEV
  ) {
    (window as unknown as { __or_audioBridge?: AudioBridge }).__or_audioBridge =
      bridge;
  }
  return bridge;
}
