...
import type { Transition, Clip, Track, Project } from "@/lib/openreel-core";
import type { TransitionType, TransitionParams } from "@/lib/openreel-core";

// Optional dirty-notification hook so transitions added/removed at runtime
// trigger auto-save. Lazy-imported via dynamic require to avoid circular
// dependencies (bridge ← auto-save ← bridge).
function notifyDirty(): void {
  try {
    // Use a dynamic import so the bridge bundle isn't tied to auto-save.
    import("../services/auto-save")
      .then((m) => m.markProjectDirty())
      .catch(() => undefined);
  } catch {
    // No-op if auto-save isn't available (tests, SSR, etc.)
  }
}
...
export class TransitionBridge {
  ...
  createTransition(
    ...
    // Notify auto-save so the new transition gets serialized on the next tick.
    notifyDirty();
  ...
  /**
   * Get all transitions across all tracks
   *
   * @returns Flat array of all transitions
   */
  ...
  /**
   * Serialize the bridge's internal transition state INTO a project model.
   * This mutates a copy of the project so each Track has its `transitions[]`
   * populated from the bridge's `trackTransitions` map.
   *
   * Called by save paths so the on-disk JSON contains transitions.
   *
   * @param project - The project to serialize transitions into
   * @returns A new project with `track.transitions` populated
   */
  serializeIntoProject(project: Project): Project {
    const updatedTracks = project.timeline.tracks.map((track) => {
      const transitions = this.trackTransitions.get(track.id) || [];
      // Always replace track.transitions with bridge state (source of truth).
      // Use [...transitions] to deep-clone the array (transition objects are
      // frozen-ish — readonly fields — but the array reference must be fresh).
      return { ...track, transitions: [...transitions] };
    });
    return {
      ...project,
  ...
  /**
   * Restore the bridge's internal transition state FROM a project model.
   * This reads `track.transitions[]` from each track and populates the
   * `trackTransitions` map. Called by load paths so reloading a project
   * restores its transitions.
   *
   * @param project - The project to restore transitions from
   */
  restoreFromProject(project: Project): void {
    if (!this.initialized || !this.transitionEngine) {
      // Late-restore: caller didn't initialize the bridge yet. Stash the
      // transitions anyway — the bridge map works without an engine for
      // pure reads, and downstream renderer paths gate on isInitialized().
    }
    // Clear existing state first so loading a project doesn't accidentally
    // merge stale transitions from a previously-open project.
    this.trackTransitions.clear();
    for (const track of project.timeline.tracks) {
      if (track.transitions && track.transitions.length > 0) {
        // Defensive clone — readonly arrays can still be passed around safely.
        this.trackTransitions.set(track.id, [...track.transitions]);
      }
    }
  }
