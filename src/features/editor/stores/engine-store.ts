...
import type { RenderedFrame } from "@/lib/openreel-core";
  ...
  subscribeWithSelector((set, get) => ({
    ...
    initialize: async () => {
      ...
      try {
        ...
        // Dev-only: expose the playback controller on `window` so Playwright
        // self-tests can attach an AnalyserNode to the audio graph's master
        // bus and measure actual output. Production bundles are guarded by
        // import.meta.env.DEV.
        if (typeof window !== "undefined" && import.meta.env?.DEV) {
          (
            window as unknown as { __or_playbackController?: unknown }
          ).__or_playbackController = playbackController;
          (
            window as unknown as { __or_engineStore?: unknown }
          ).__or_engineStore = useEngineStore;
        }
