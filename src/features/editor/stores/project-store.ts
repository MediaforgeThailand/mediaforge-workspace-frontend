 ...
 * survive a page reload — blob: URLs are invalidated when the page
 * unloads, but data: URLs are pure strings serializable in the project
 * JSON.
 */
...
export interface ProjectState {
  ...
  /**
   * Detach the audio of a video clip onto a new audio track.
   *
   * Industry-standard "Detach audio" — splits the source video clip into a
   * video-only clip (with volume muted) plus a new audio-only clip on a new
   * audio track at the same start time. Subsequent edits to the two clips
   * are independent.
   */
  detachAudio: (clipId: string) => Promise<ActionResult>;
  /**
   * Update a clip's volume (linear 0..4 — 1.0 = unity gain).
   * Used by the timeline waveform-drag UI and the inspector slider.
   */
  ...
  updateClipBlendMode: (
    clipId: string,
    blendMode: import("@/lib/openreel-core").BlendMode,
  ) => boolean;
  ...
  updateClipEmphasisAnimation: (
    clipId: string,
    emphasisAnimation: import("@/lib/openreel-core").EmphasisAnimation,
  ) => boolean;
  ...
  /**
   * Create a caption text clip with word-level timing + group metadata.
   * Used by AI Captions feature.
   */
  ...
  applyTextAnimationPreset: (
    ...
    outPreset?: TextAnimationPreset,
  ) => TextClip | null;
  getAvailableAnimationPresets: () => TextAnimationPreset[];

  // Subtitle actions - subtitles are created as text clips on a Captions track
  addSubtitle: (subtitle: import("@/lib/openreel-core").Subtitle) => Promise<void>;
  removeSubtitle: (subtitleId: string) => void;
  updateSubtitle: (
    subtitleId: string,
    updates: Partial<import("@/lib/openreel-core").Subtitle>,
  ) => void;
  getSubtitle: (
    subtitleId: string,
  ) => import("@/lib/openreel-core").Subtitle | undefined;
  importSRT: (
    srtContent: string
  ...
  updateMarker: (
    markerId: string,
    updates: Partial<import("@/lib/openreel-core").Marker>,
  ) => void;
  getMarker: (markerId: string) => import("@/lib/openreel-core").Marker | undefined;
  getMarkers: () => import("@/lib/openreel-core").Marker[];
  ...
  updateSVGClip: (
    ...
    updates: {
      ...
      duration?: number;
      transform?: Partial<Transform>;
      entryAnimation?: import("@/lib/openreel-core").GraphicAnimation;
      exitAnimation?: import("@/lib/openreel-core").GraphicAnimation;
      colorStyle?: import("@/lib/openreel-core").SVGColorStyle;
    },
  ) => SVGClip | null;
  ...
  subscribeWithSelector((set, get) => {
    ...
    return {
      ...
      loadProject: (project: Project) => {
        ...
        // V6-C1: Restore transitions from project model into the bridge.
        // The bridge is the runtime source of truth for transitions; the
        // project model holds the serialized state. Without this, transitions
        // persisted to disk wouldn't render after reload.
        try {
          getTransitionBridge().restoreFromProject(fixedProject);
        } catch (e) {
          console.warn("[ProjectStore] transition restore failed:", e);
        }
        ...
        try {
          ...
          if (
            ...
            for (const thumb of processedMedia.thumbnails) {
              ...
              if (thumb.dataUrl) {
                ...
                // Convert canvas to a base64 dataUrl. Filmstrips need to
                // survive a page reload, so we never use blob: URLs here
                // (they're invalidated when the page unloads). Cost is a
                // ~30% size increase vs blob URLs but we get persistence.
                try {
          ...
          const newMediaItem: MediaItem = {
            ...
            metadata: {
              ...
              // Preserve audioTrackCount so ClipMediaCanvas can detect audio
              // even when a transmuxed file leaves `channels` at 0 but the
              // ffmpeg probe found tracks. Without this the waveform area
              // would hide on those clips.
          ...
          if (
            ...
            for (const thumb of processedMedia.thumbnails) {
              ...
              if (thumb.dataUrl) {
                ...
                try {
                  if (thumb.canvas instanceof OffscreenCanvas) {
                    // Use data URL not blob URL so filmstrips survive reload.
                    const blob = await thumb.canvas.convertToBlob({
                      type: "image/jpeg",
                      quality: 0.7,
                    });
      ...
      addClipToNewTrack: async (mediaId: string, startTime?: number) => {
        ...
        // Snapshot existing track ids so we can pick the *new* track by
        // exclusion after addTrack completes. Previously this was a fragile
        // `find(empty && type)` which would pick the first empty track of
        // the right type — wrong if a pre-existing empty track of the same
        // type sat earlier in the array (works today by accident because
        // addTrack inserts at top, but breaks the moment that default
        // changes).
        ...
        const newTrack = updatedProject.timeline.tracks.find(
          (t) => t.type === trackType && !existingIds.has(t.id),
        );
      ...
      /**
       * Detach audio from a video clip — creates a new audio track + clip and
       * mutes the source video clip's audio output.
       *
       * Effectively `separateAudio` plus "set source clip volume → 0" so the
       * new audio clip is the only one rendering audio. Both ops are committed
       * in a single project update so undo restores the original state in one
       * step.
       */
      detachAudio: async (clipId: string) => {
        const { project, actionExecutor } = get();
        ...
        if (
          ...
          return {
            ...
            error: {
              code: "MEDIA_NOT_FOUND" as const,
              message: "Media has no audio to detach",
            },
          };
        }

        // Apply all changes on a single project copy so undo treats this as
        // one atomic edit.
        const projectCopy = structuredClone(project);

        // 1. Add a new audio track at the bottom.
        const newAudioTrackId = uuidv4();
        const trackAction: Action = {
          type: "track/add",
          id: uuidv4(),
          timestamp: Date.now(),
          params: { trackType: "audio", trackId: newAudioTrackId },
        };
        const trackResult = await actionExecutor.execute(trackAction, projectCopy);
        if (!trackResult.success) {
          return {
        ...
        // The created audio track id may be assigned by the executor — find the
        // newly-added track of type "audio".
        const audioTracks = projectCopy.timeline.tracks.filter(
          (t) => t.type === "audio",
        );
        const targetTrack = audioTracks[audioTracks.length - 1];
        if (!targetTrack) {
          return {
            success: false,
            error: {
              code: "TRACK_NOT_FOUND" as const,
              message: "Could not find newly-added audio track",
            },
          };
        }

        // 2. Add an audio clip mirroring the source video clip's timing/trim.
        const clipAction: Action = {
          ...
          params: {
            ...
            mediaId: videoClip.mediaId,
            startTime: videoClip.startTime,
            audioTrackIndex: videoClip.audioTrackIndex ?? 0,
          },
        };
        const clipResult = await actionExecutor.execute(clipAction, projectCopy);
        if (!clipResult.success) {
          return clipResult;
        }

        // The new clip will adopt the media's full duration — re-trim it to
        // match the source video clip's in/out so the audio lines up exactly.
        const newClip = targetTrack.clips[targetTrack.clips.length - 1];
        if (newClip) {
          const trimAction: Action = {
            type: "clip/trim",
            id: uuidv4(),
            timestamp: Date.now(),
            params: {
          ...
          await actionExecutor.execute(trimAction, projectCopy);
        }

        // 3. Mute the source video clip's audio so it doesn't double with the
        // new detached audio clip.
        const muteAction: Action = {
          type: "audio/setVolume",
          id: uuidv4(),
          timestamp: Date.now(),
          params: { clipId: videoClip.id, volume: 0 },
        };
        await actionExecutor.execute(muteAction, projectCopy);
      ...
      /**
       * Set a clip's volume (linear 0..4).
       *
       * Mirrors the live-drag path used by the inspector AudioBasicTab —
       * direct project tree update for snappy UI feedback. Volume drag UIs
       * call this during mousemove and again on mouseup. Undo coverage for
       * volume changes is provided by the action history via the audio/setVolume
       * action used by other paths (separateAudio / detachAudio).
       */
      ...
      duplicateClip: async (clipId: string) => {
        ...
        // Snapshot existing ids so we can pick the new track by exclusion —
        // safer than `find(empty && type)` which would silently pick the
        // wrong track if any pre-existing empty track of the same type sat
        // earlier in the array.
        const existingTrackIds = new Set(project.timeline.tracks.map((t) => t.id));
        ...
        const newTrack = updatedProject.timeline.tracks.find(
          (t) => t.type === track.type && !existingTrackIds.has(t.id),
        );
      ...
      createTextClip: (
        ...
        // Validate inputs — silently clamping NaN/negative was a real bug
        // surfaced in the V6 audit. We reject obviously-invalid clip params
        // here so the project state can never carry corrupt clips that crash
        // the renderer or break save/load round-trips.
        if (!Number.isFinite(startTime) || startTime < 0) {
          console.error(
            `createTextClip: invalid startTime ${startTime} — must be finite and >= 0`,
          );
          return null;
        }
        if (!Number.isFinite(duration) || duration <= 0) {
          console.error(
            `createTextClip: invalid duration ${duration} — must be finite and > 0`,
          );
          return null;
        }
      ...
      /**
       * Create a caption text clip with word-level timing and group metadata.
       * Used by the AI Captions feature. Skips the undo stack because captions
       * are usually created in a batch — undoing one at a time would be a poor
       * UX (group-delete is exposed instead).
       */
      createCaptionTextClip: (options) => {
        const titleEngine = useEngineStore.getState().titleEngine;
        if (!titleEngine) {
          console.error("TitleEngine not available yet");
          return null;
        ...
        const { project } = get();
        const track = project.timeline.tracks.find((t) => t.id === options.trackId);
        if (!track) {
          console.error(`Track ${options.trackId} not found`);
          return null;
        }
        // Mirror createTextClip's V6 validation — Whisper has been observed
        // to occasionally emit segments with NaN or negative times when the
        // input clip starts mid-word. Reject those at the boundary instead
        // of letting them poison the project state.
        if (!Number.isFinite(options.startTime) || options.startTime < 0) {
          console.error(
            `createCaptionTextClip: invalid startTime ${options.startTime}`,
          );
          return null;
        }
        if (!Number.isFinite(options.duration) || options.duration <= 0) {
          console.error(
            `createCaptionTextClip: invalid duration ${options.duration}`,
          );
          return null;
        }
        const textClip = titleEngine.createTextClip({
      ...
      applyTextAnimationPreset: (
        ...
        const baseAnimation = textAnimationEngine.createAnimationPreset(
          preset,
          inDuration,
          outDuration,
          params,
        );
        // Attach `outPreset` separately. The engine treats `outPreset ===
        // preset` (or undefined) as "use the same animation in both phases"
        // — matching pre-V6.5 behavior — so this is backwards-compatible.
        const animation = outPreset
          ? { ...baseAnimation, outPreset }
          : baseAnimation;
      ...
      addSubtitle: async (subtitle) => {
        ...
        // V6-M2: Unify with CaptionsPanel — both must use the same "find
        // captions track" strategy. Strategy B (first-text-track) matches
        // CapCut / Descript: any text track is fair game; captions and
        // titles share the track type. captionMeta on the clip still
        // distinguishes captions from ordinary text clips for rendering.
        let captionsTrack = project.timeline.tracks.find(
          (t) => t.type === "text"
        );

        if (!captionsTrack) {
          // Snapshot existing track ids before addTrack to avoid the fragile
          // "find first empty text track" pattern (V6-H3).
          const existingTrackIds = new Set(project.timeline.tracks.map((t) => t.id));
          ...
          captionsTrack = updatedProject.timeline.tracks.find(
            (t) => t.type === "text" && !existingTrackIds.has(t.id)
          );
        }
      ...
      addMarker: (time, label = "Marker", color = "#3b82f6") => {
        const newMarker: import("@/lib/openreel-core").Marker = {
...
// Dev-only: expose the project store on `window` so Playwright self-tests can
// seed media/clips directly without a full file-import dance. Guarded by
// import.meta.env.DEV so production bundles do not leak internals.
if (typeof window !== "undefined" && import.meta.env.DEV) {
  (window as unknown as { __or_projectStore?: typeof useProjectStore }).__or_projectStore = useProjectStore;
}
