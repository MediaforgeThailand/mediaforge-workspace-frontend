...
// Editor tool modes — "select" is the default arrow cursor (CapCut "A" tool).
// "split" and "trim" are reserved for future tool implementations.
export type ToolMode = "select" | "split" | "trim";
...
export interface TimelineState {
  ...
  // CapCut-parity toggles (P / N / Shift+L / Alt+P)
  magnet: boolean;
  linkage: boolean;
  previewAxis: boolean;
  toolMode: ToolMode;
  previewAxisTime: number | null;
  play: () => void;
  pause: () => void;
  stop: () => void;
  togglePlayback: () => void;
  ...
  toggleLinkage: () => void;
  setLinkage: (enabled: boolean) => void;
  togglePreviewAxis: () => void;
  setPreviewAxis: (enabled: boolean) => void;
  setPreviewAxisTime: (time: number | null) => void;
...
export const useTimelineStore = create<TimelineState>()(
  subscribeWithSelector(persist((set, get) => ({
    ...
    viewportWidth: 800,
    viewportHeight: 400,
    // Default = 50 (compact tracks). The denser 50-px row fits more layers
    // on screen without scrolling and matches the timeline-toolbar's Rows2
    // toggle being active by default. Users can switch to 80-px (Rows3)
    // tracks via the toolbar.
    trackHeight: 50,
    trackHeights: {},
    ...
    // CapCut-parity defaults: magnet/linkage/preview-axis all on, snapping
    // managed by UIStore.snapSettings.enabled.
    magnet: true,
    linkage: true,
    previewAxis: true,
    toolMode: "select" as ToolMode,
    previewAxisTime: null,
    ...
    setPlaybackRate: (rate: number) => {
      // Allow negative rates for J shuttle (CapCut-style reverse playback)
      const sign = rate < 0 ? -1 : 1;
      const abs = Math.max(0.1, Math.min(4.0, Math.abs(rate)));
      set({ playbackRate: sign * abs });
    },
    ...
    toggleMagnet: () => {
      set((state) => ({ magnet: !state.magnet }));
    },
    setMagnet: (enabled: boolean) => {
      set({ magnet: enabled });
    },
    toggleLinkage: () => {
      set((state) => ({ linkage: !state.linkage }));
    },
    setLinkage: (enabled: boolean) => {
      set({ linkage: enabled });
    },
    togglePreviewAxis: () => {
      set((state) => ({ previewAxis: !state.previewAxis }));
    },
    setPreviewAxis: (enabled: boolean) => {
      set({ previewAxis: enabled });
    },
    setPreviewAxisTime: (time: number | null) => {
      set({ previewAxisTime: time });
    },
    setToolMode: (mode: ToolMode) => {
      set({ toolMode: mode });
    },
  }), {
    name: "openreel-timeline-toggles",
    version: 1,
    // Persist only the CapCut-parity toggles; everything else (playhead,
    // zoom, scroll, viewport, loop state, scrubbing, expansion sets,
    // tool mode) is intentionally ephemeral.
    partialize: (state) => ({
      magnet: state.magnet,
      linkage: state.linkage,
      previewAxis: state.previewAxis,
    }),
  })),
);

// Dev-only: expose the timeline store so Playwright self-tests can poke at
// zoom / playhead state directly. Production bundles strip this.
if (typeof window !== "undefined" && import.meta.env.DEV) {
  (window as unknown as { __or_timelineStore?: typeof useTimelineStore }).__or_timelineStore = useTimelineStore;
}
