import type { Project, ProjectSettings } from "@/lib/openreel-core";
import { v4 as uuidv4 } from "uuid";
import { getTransitionBridge } from "../bridges/transition-bridge";

/**
 * Serialize bridge-managed state (transitions etc.) into the project model
 * before persisting to disk. Without this, transitions live only in the
 * bridge's in-memory map and disappear on reload (V6-C1).
 */
function serializeBridgeState(project: Project): Project {
  try {
    return getTransitionBridge().serializeIntoProject(project);
  } catch (e) {
    console.warn("[ProjectManager] serializeBridgeState failed:", e);
    return project;
  }
}

/**
 * Restore bridge-managed state (transitions etc.) from a loaded project model.
 * Called on every load path so the bridge map is in sync with the project.
 */
function restoreBridgeState(project: Project): void {
  try {
    getTransitionBridge().restoreFromProject(project);
  } catch (e) {
    console.warn("[ProjectManager] restoreBridgeState failed:", e);
  }
}
...
class ProjectManager {
  ...
  async saveProject(project: Project): Promise<boolean> {
    const enrichedProject = serializeBridgeState(project);
    if (this.currentFileHandle) {
      return this.saveToFileHandle(enrichedProject, this.currentFileHandle);
    }
    return this.saveProjectAs(enrichedProject);
  }

  async saveProjectAs(project: Project): Promise<boolean> {
    const enrichedProject = serializeBridgeState(project);
    if (!("showSaveFilePicker" in window)) {
      return this.downloadProject(enrichedProject);
    }

    try {
      const win = window as WindowWithFilePicker;
      const handle = await win.showSaveFilePicker!({
        suggestedName: `${enrichedProject.name}.oreel`,
        types: [
          {
            description: "MediaForge Project",
            accept: { "application/json": [".oreel", ".json"] },
          },
        ],
      });

      const success = await this.saveToFileHandle(enrichedProject, handle);
      if (success) {
        this.currentFileHandle = handle;
        await this.addToRecent(enrichedProject, handle);
      }
      return success;
    } catch (error) {
