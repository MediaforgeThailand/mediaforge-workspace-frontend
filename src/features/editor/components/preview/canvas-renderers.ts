...
/**
 * Apply the CSS-style textTransform field to a raw string. Returns the
 * transformed string ready to render. We do this on the rendered string only
 * — the underlying clip text is never mutated, so the user's content stays
 * intact when they toggle between case modes.
 */
const applyTextTransform = (
  s: string,
  mode: "none" | "uppercase" | "lowercase" | "capitalize" | undefined,
): string => {
  if (!mode || mode === "none") return s;
  if (mode === "uppercase") return s.toUpperCase();
  if (mode === "lowercase") return s.toLowerCase();
  // Title-case: capitalize first letter of each word, leave the rest as-is.
  // We deliberately don't lowercase the rest so existing emphasis (e.g. ALL
  // CAPS within a word) survives.
 ...
 * a bubble. `none` returns false so the caller falls through to the normal
 * backgroundColor flow.
 */
const drawTextBubble = (
  ctx: CanvasRenderingContext2D,
  style: import("@/lib/openreel-core").BubbleStyle,
  width: number,
  height: number,
  fillColor: string,
): boolean => {
  ...
  if (style === "rounded-rect") {
    ...
    // Rounded rect + tail at bottom-left
    const r = Math.min(20, h / 2);
    ctx.beginPath();
    ctx.moveTo(x + r, y);
    ctx.lineTo(x + w - r, y);
    ...
    ctx.lineTo(x + w, y + h - r);
    ctx.quadraticCurveTo(x + w, y + h, x + w - r, y + h);
    ctx.lineTo(x + r + 30, y + h);
    ctx.lineTo(x + r + 10, y + h + 20);
    ctx.lineTo(x + r + 20, y + h);
    ctx.lineTo(x + r, y + h);
    ctx.quadraticCurveTo(x, y + h, x, y + h - r);
    ctx.lineTo(x, y + r);
    ctx.quadraticCurveTo(x, y, x + r, y);
    ...
    // Ellipse + small bubbles below
    ctx.beginPath();
    ctx.ellipse(0, 0, w / 2, h / 2, 0, 0, Math.PI * 2);
    ctx.fill();
    ctx.beginPath();
    ctx.arc(-w / 4, h / 2 + 10, 6, 0, Math.PI * 2);
    ctx.fill();
    ctx.beginPath();
    ctx.arc(-w / 4 - 14, h / 2 + 22, 4, 0, Math.PI * 2);
    ctx.fill();
  } else if (style === "cloud") {
    // Series of overlapping circles approximating a cloud
    const cx = 0;
    const cy = 0;
    const radii = [
      { dx: -w * 0.3, dy: 0, r: h * 0.6 },
      { dx: -w * 0.1, dy: -h * 0.15, r: h * 0.7 },
      { dx: w * 0.1, dy: -h * 0.05, r: h * 0.6 },
      { dx: w * 0.3, dy: 0.05, r: h * 0.55 },
      { dx: 0, dy: h * 0.15, r: h * 0.55 },
    ];
    ...
    // 5-pointed star sized to fit
    const outer = Math.min(w, h * 1.8) / 2;
...
export const renderTextClipToCanvas = (
  ...
  // fields if the effects bag has no shadow entry (backward compatibility).
  const effects = style.effects;

  // Shadow — prefer the new effects.shadow if enabled, else the legacy fields.
  ...
  } else if (style.shadowColor && style.shadowBlur) {
  ...
  // Measure the longest line so we can draw an enclosing bubble / background.
  // Letter-spacing isn't honored by ctx.measureText, so we add a rough
  // per-character pad to approximate it.
  ...
  if (style.bubbleStyle && style.bubbleStyle !== "none") {
    const bubbleColor =
      effects?.background?.enabled && effects.background.color
        ? effects.background.color
        : "rgba(255,255,255,0.95)";
    const padX = Math.max(40, style.fontSize * 0.8);
    const padY = Math.max(24, style.fontSize * 0.5);
    drawTextBubble(
      ctx,
      style.bubbleStyle,
      maxLineWidth + padX,
  ...
  } else if (effects?.background?.enabled) {
    // Solid rectangle background when no bubble shape is chosen.
    const bg = effects.background;
    const padX = 16;
    const padY = 8;
    const bw = maxLineWidth + padX;
    const bh = totalHeight + padY;
    ctx.save();
    ctx.fillStyle = bg.color;
    const r = Math.min(bg.cornerRadius || 0, bh / 2);
    if (r > 0) {
      const bx = -bw / 2;
      const by = -bh / 2;
      ctx.beginPath();
  ...
  // Glow — drawn as an extra stroke + shadow pass before the main text
  // so it sits behind. We do it in a save/restore so it doesn't leak.
  const drawGlowIfEnabled = (drawText: () => void): void => {
  ...
  const outline = effects?.outline?.enabled ? effects.outline : null;
  ...
  if (characterStates && characterStates.length > 0) {
    ...
    if (wantsWordHighlight) {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        // Walk the line word-by-word, matching against textClip.words[] to
        // determine which color to use for each word. Whisper preserves
        ...
        for (let tIdx = 0; tIdx < tokens.length; tIdx++) {
          ...
          const meta = textClip.words?.[wordCursor];
          wordCursor++;

          const isActive = !!(
            meta && time >= meta.start && time < meta.end
          );
          ...
          // Fill pass for word in correct color
          ctx.save();
          ctx.fillStyle = color;
          ctx.textAlign = "left";
          ctx.fillText(token, cursorX, y);
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // effects.outline. The new one wins when both are present.
      if (outline && outline.width > 0) {
        ctx.save();
        ctx.strokeStyle = outline.color;
        ctx.lineWidth = outline.width;
      ...
      if (style.textDecoration === "underline") {
        ...
        ctx.beginPath();
        const underlineY = y + style.fontSize * 0.5;
...
export const drawFrameWithTransform = (
  ...
  canvasWidth: number,
  canvasHeight: number,
  // Optional compositing — when the clip has a non-"normal" blendMode the
  // caller passes it here so the frame is composited onto the canvas using
  // that mode (multiply / screen / overlay etc). `blendOpacity` is the
  // per-clip percent (0..100) and gets multiplied into the transform opacity.
  blendOptions?: {
  ...
  const extraOpacity =
    blendOptions?.blendOpacity !== undefined
      ? Math.max(0, Math.min(1, blendOptions.blendOpacity / 100))
      : 1;
  ctx.globalAlpha = baseOpacity * extraOpacity;
  if (blendOptions?.blendMode && blendOptions.blendMode !== "normal") {
    ctx.globalCompositeOperation = blendOptions.blendMode as GlobalCompositeOperation;
  }
