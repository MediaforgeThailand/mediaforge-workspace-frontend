...
export const BlendingSection: React.FC<BlendingSectionProps> = ({ clipId }) => {
  ...
  // RAF-batched on drag — slider updates render preview live (cheap), and
  // we only push to the project store on release so undo history holds
  // one entry per drag instead of dozens.
  const handleOpacityChange = useCallback(
    (opacity: number) => {
      // Live preview path: write the in-flight opacity to the renderer so
      // the user sees the result of the drag. updateClipBlendOpacity itself
      // already commits to the project; that's fine — RAF batching inside
      // LabeledSlider ensures we run at most once per frame.
  ...
  const handleOpacityCommit = useCallback(
    (opacity: number) => {
      // Final commit — the live path already wrote, this is here so the
      // slider treats release as "done dragging" and disposes its RAF.
      updateClipBlendOpacity(clipId, opacity);
    },
    [clipId, updateClipBlendOpacity],
  );
