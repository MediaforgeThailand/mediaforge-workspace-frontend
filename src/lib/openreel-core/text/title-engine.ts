...
export interface CreateTextClipOptions {
  ...
  /** Word-level timestamps for caption rendering (optional). */
  words?: ReadonlyArray<{ text: string; start: number; end: number }>;
  /** Caption group metadata (optional). */
  captionMeta?: CaptionClipMeta;
}
...
export class TitleEngine {
  ...
  updateTextClip(
    ...
    const updatedClip: TextClip = {
      ...
      captionMeta: updates.captionMeta ?? existing.captionMeta,
    };
