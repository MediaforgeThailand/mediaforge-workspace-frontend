...
export class TextAnimationEngine {
  ...
  getAnimatedState(clip: TextClip, time: number): AnimatedTextState {
    ...
    // Distinct In/Out presets: if `outPreset` is set and we're in the Out
    // phase (`outProgress > 0`), evaluate the Out preset with the clip
    // fully "in" (inProgress=1) so the Out animation runs unimpeded. The
    // In phase runs the regular preset with outProgress=0. During the
    // Hold phase both progresses are at their resting values so either
    // path produces the same result.
    if (animation.outPreset && animation.outPreset !== animation.preset && outProgress > 0) {
