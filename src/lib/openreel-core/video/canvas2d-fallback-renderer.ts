...
export class Canvas2DFallbackRenderer implements Renderer {
  ...
  /**
   * Canvas2D fallback: there's no GPU adapter behind us. The browser may
   * still composite onto a GPU surface, but our pixel work is CPU-side.
   * We mark `isHardwareAccelerated: false` so the UI can say "CPU-only".
   */
