...
export class WebGPURenderer implements Renderer {
  ...
  /**
   * Surface adapter metadata for the UI's "Local rendering" tooltip / panel.
   * `adapter.info` is the modern API; some browsers ship `adapter.info`
   * synchronously while others (Safari TP, older Chromium) expose
   * `requestAdapterInfo()` instead. We feature-detect both. If neither is
   * available we still report which renderer is active so the UI can show
  ...
  getAdapterInfo(): RendererAdapterInfo {
    ...
    // `info` is the standardized synchronous property. We cast through
    // unknown because TypeScript's lib.dom may not have it yet. Inline
    // the shape so we don't depend on a global GPUAdapterInfo type.
    type RawAdapterInfo = {
      vendor?: string;
      architecture?: string;
      device?: string;
    ...
    const rawInfo = (this.adapter as unknown as { info?: RawAdapterInfo }).info;
    const limits = this.adapter.limits;

    return {
      type: "webgpu",
      vendor: rawInfo?.vendor || undefined,
      architecture: rawInfo?.architecture || undefined,
      device: rawInfo?.device || undefined,
      description: rawInfo?.description || undefined,
      // `isFallbackAdapter` is a different concept than compatibility mode,
      // but it's the closest standardised flag we have. Browsers that
      // surface `featureLevel === "compatibility"` would also live here
      // once the API stabilises. Cast through unknown because not every
      // tsconfig in the monorepo has the latest lib.webgpu types.
      isCompatibilityMode:
        (this.adapter as unknown as { isFallbackAdapter?: boolean })
          .isFallbackAdapter ?? false,
      maxTextureDimension2D: limits?.maxTextureDimension2D,
      isHardwareAccelerated: true,
      adapterInfoAvailable: !!rawInfo,
    };
  }
