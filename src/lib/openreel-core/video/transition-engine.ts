...
export class TransitionEngine {
  ...
  validateTransition(
    ...
    // Delta > 0 = gap, delta < 0 = overlap, delta == 0 = exactly adjacent
    const delta = clipB.startTime - clipAEnd;
    const EPS = 0.001;

    // V6-M3: Allow OVERLAPPING clips. Standard NLE behavior (CapCut /
    // Premiere / FCP): overlap region IS the transition zone. Only reject
    // when there's a real gap between the clips.
    if (delta > EPS) {
      return {
        valid: false,
    ...
    // If clips overlap, the overlap region is a natural maximum for the
    // transition (no point in transitioning past where the other clip
    // already plays).
    const overlap = delta < -EPS ? -delta : Infinity;
  ...
  areClipsAdjacent(clipA: Clip, clipB: Clip): boolean {
    ...
    // V6-M3: Treat touching AND overlapping clips as "adjacent" for the
    // purposes of allowing transitions. Only a real positive gap rejects.
    return delta < 0.001;
  }
