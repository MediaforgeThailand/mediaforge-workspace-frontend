...
interface ScheduledSource {
  ...
  /** Per-clip GainNode for live volume changes during playback. */
  clipGain: GainNode;
  startedAt: number;
  duration: number;
}
...
export class RealtimeAudioGraph {
  ...
  /**
   * Most-recent per-clip volume overrides set via {@link setClipVolume}.
   * Applied to newly scheduled clips so a volume change made *before* playback
   * is honoured the moment the scheduler creates the BufferSource.
   */
  ...
  /**
   * Update the per-clip gain in real-time during playback.
   *
   * The clip's GainNode is looked up across all tracks (each scheduled clip
   * owns its own gain node — see {@link scheduleClip}). The new value is
   * applied via `setTargetAtTime` with a very short time-constant so there's
   * no zipper noise during a rubberband drag, yet the response is effectively
   * instant (< 30 ms to reach the target). For true silence (volume 0 or
   * sub-audible levels) we use `setValueAtTime` to hit zero immediately —
   * `setTargetAtTime` approaches asymptotically and never actually reaches 0.
   *
   * Even if the clip has not yet been scheduled (e.g. user scrubs the volume
   * before playback starts), the override is stored and applied when
   * scheduling happens.
   */
  setClipVolume(clipId: string, volume: number): void {
    const clamped = Math.max(0, Math.min(4, volume));
    this.clipVolumeOverrides.set(clipId, clamped);

    for (const [, sources] of this.scheduledSources) {
      for (const scheduled of sources) {
        if (scheduled.clipId !== clipId) continue;
        const gain = scheduled.clipGain.gain;
        const now = this.audioContext.currentTime;
        if (clamped <= 0.0001) {
          // Hard mute: setTargetAtTime never reaches 0, so use setValueAtTime
          // to land on exact zero. Cancel any in-flight ramp first.
          gain.cancelScheduledValues(now);
          gain.setValueAtTime(0, now);
        } else {
          // Smooth ramp (~10 ms time constant) prevents zipper noise on drag.
          gain.cancelScheduledValues(now);
          gain.setTargetAtTime(clamped, now, 0.01);
        }
      }
    }
  }

  getClipVolume(clipId: string): number {
    return this.clipVolumeOverrides.get(clipId) ?? 1;
  }
  ...
  scheduleClip(schedule: AudioClipSchedule): void {
    ...
    // Honour any override that was set before the clip was scheduled (e.g.
    // user dragged the rubberband while paused, then pressed play). Without
    // this, the volume jumps back to the value baked into the schedule for
    // a beat before the subscription catches up.
    const effectiveVolume = this.clipVolumeOverrides.has(schedule.clipId)
      ? this.clipVolumeOverrides.get(schedule.clipId)!
      : schedule.volume;
  ...
  dispose(): void {
    ...
    this.clipVolumeOverrides.clear();
