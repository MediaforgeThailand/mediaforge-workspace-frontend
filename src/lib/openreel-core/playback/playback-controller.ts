...
export class PlaybackController {
  ...
  /**
   * When true, the controller's per-tick frame render is suppressed because
   * an external renderer (e.g. the Preview React component's RAF loop) owns
   * the canvas. The controller still drives the master clock and emits
   * `timeupdate` events so subscribers can sync, but does NOT call
   * `videoEngine.renderFrame` on every tick.
   *
   * Without this flag, the controller's render path and the Preview's
   * multi-track loop both invoke MediaBunny on every clock tick, doubling
   * decoder pressure and starving the actually-visible loop of decoder time.
   * See the multi-track perf bug for the full backstory.
   */
  private externalRendererActive: boolean = false;
  ...
  private handleClockTimeUpdate(time: number): void {
    ...
    // When an external renderer is active, we skip the controller's
    // per-tick render. This avoids the controller and the Preview loop
    // both decoding frames on every clock tick. The external renderer is
    // responsible for drawing; we just keep subscribers in sync via
    // timeupdate events.
    if (!this.externalRendererActive) {
  ...
  /**
   * Mark that an external renderer (e.g. the Preview RAF loop) owns the
   * canvas. While set, the controller skips its per-tick `videoEngine.
   * renderFrame` to avoid contending for decoder time. Scrub / seek still
   * render through the controller (for parked frames) — only the
   * play-loop render is suppressed.
   */
  setExternalRendererActive(active: boolean): void {
    this.externalRendererActive = active;
  }

  /** Returns whether an external renderer is currently driving display. */
  isExternalRendererActive(): boolean {
    return this.externalRendererActive;
  }
