  ...
  private applyTrackAction(
    ...
    switch (action.type) {
      case "track/add": {
        ...
        // Default insert position: TOP of the visual group the track belongs
        // to. Standard NLE convention:
        //   - Audio tracks live in their own group at the bottom of the stack.
        //     A new audio track inserts at the top of the audio group (so it
        //     ends up just below all video/text/graphics tracks).
        //   - Everything else (video / text / graphics / image) inserts at
        //     position 0 — the very top — which gives the new layer the
        //     highest z-order in the preview composite.
        // Explicit position arg still wins.
        let position: number;
        if (params.position !== undefined) {
          position = params.position;
        } else if (params.trackType === "audio") {
          // First index whose track type is "audio" — new audio track lands
          // at top of audio group. If no audio yet, falls through to the end.
          const firstAudioIdx = timeline.tracks.findIndex(
            (t: MutableTrack) => t.type === "audio",
          );
          position =
            firstAudioIdx === -1 ? timeline.tracks.length : firstAudioIdx;
        } else {
          position = 0;
        }
